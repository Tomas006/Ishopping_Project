    ...
    public static class FormPlanearListaController
    {
        ...
        public static string GravarPlaneamento(string nomeLista, List<ItemPrevisto> itens, int idUtilizador)
        {
            try
            {
                using (var db = new IShoppingContext())
                {
                    var dicionarioArtigos = db.Artigos.ToDictionary(a => a.Id, a => a);
                    ...
                    {
                        string nomeReal = "Artigo Desconhecido";
                        decimal precoReal = 0m;

                        if (dicionarioArtigos.ContainsKey(itemEmMemoria.ArtigoId))
                        {
                            nomeReal = dicionarioArtigos[itemEmMemoria.ArtigoId].Nome;
                            precoReal = dicionarioArtigos[itemEmMemoria.ArtigoId].Preco;
                        }
        ...
        public static string AtualizarPlaneamento(int compraId, string nomeLista, List<ItemPrevisto> itens, int idUtilizador)
        {
            try
            {
                using (var db = new IShoppingContext())
                {
                    ...
                    compraExistente.Nome = nomeLista; 
